...
else if($postjson['requisicao']=='chamado'){
    $query = $pdo->prepare("INSERT INTO servico SET descricao=:descricao, data=:data,Cliente_id= :Cliente_id, valor=:valor");
    $query->bindValue(":descricao",$postjson['descricao']);
    $query->bindValue(":data",$postjson['data']);
    $busca->bindValue(":Cliente_id",$postjson['Cliente_id']);
    $query = $pdo->query("SELECT id FROM cliente WHERE id LIKE '$busca' order by id desc");
